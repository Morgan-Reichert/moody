...
import { dueReminders, expiryReminders, DueReminder, Alarm, notify, vibrate } from "@/lib/reminders";
import { expiringDocs, DocMeta } from "@/lib/vault-db";
import { BarcodeScanner } from "@/components/BarcodeScanner";
import { Portal } from "@/components/Portal";
import { Smile, Pill, ScanLine, Check, Clock, CalendarClock, FileClock } from "lucide-react";
...
export function ReminderEngine({ onOpenMood }: { onOpenMood: () => void }) {
  ...
  // Scheduler loop.
  useEffect(() => {
    const loadDocs = () => { expiringDocs().then((d) => { docsRef.current = d; }).catch(() => {}); };
    loadDocs();
    const docsTimer = window.setInterval(loadDocs, 5 * 60000);
    const check = () => {
      if (activeRef.current) return;                 // one alarm at a time
      const settings = getSettings();
      const meds = getMeds();
      const now = new Date();
      const due = [...dueReminders(settings, meds, now), ...expiryReminders(docsRef.current, now)];
      for (const d of due) {
    ...
    const onVis = () => { if (document.visibilityState === "visible") { loadDocs(); check(); } };
    document.addEventListener("visibilitychange", onVis);
    return () => { window.clearInterval(id); window.clearInterval(docsTimer); document.removeEventListener("visibilitychange", onVis); };
    ...
    <div className="fixed inset-0 z-[70] flex flex-col items-center justify-center p-6 bg-ink/70 backdrop-blur-md">
      <div className="w-full max-w-sm rounded-4xl bg-cream p-7 text-center shadow-pill animate-pop">
        <div className="mx-auto grid place-items-center h-20 w-20 rounded-full bg-brand-500 text-white animate-pulseRing shadow-glow">
          {isMed ? <Pill className="h-9 w-9" /> : active.kind === "appt" ? <CalendarClock className="h-9 w-9" /> : active.kind === "expiry" ? <FileClock className="h-9 w-9" /> : <Smile className="h-9 w-9" />}
        </div>
          ...
          {requireScan ? (
            ...
            <>
              <button onClick={validate}
                className="w-full rounded-3xl py-4 bg-brand-500 text-white font-display text-[17px] font-semibold flex items-center justify-center gap-2 shadow-glow active:scale-[.99]">
                <Check className="h-5 w-5" strokeWidth={2.6} /> {isMed ? "J'ai pris" : active.kind === "mood" ? "Noter mon humeur" : "Compris"}
              </button>
              {(active.kind === "mood" || active.kind === "med") && (
